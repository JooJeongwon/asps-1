...
접속하면 Monogatari 2.8.0으로 코코네 강의실 문 앞 프롤로그가 바로 시작된다. A 박진환(왕), B 주정원(츤데레), C 정치훈(프랑스 귀족), D 남성수(운명 연구자)를 원하는 순서로 만나고, 각 인물과 두 번의 2지선다 대화를 진행한다. 모든 대답은 다음 질문으로 합류한다.
...
[3분 발표 진행 대본](docs/presentation-3min.md) · [전체 분기 대본](docs/scenario-kokone.md) · [엔진 및 편집 안내](docs/design-reference.md) · [캠퍼스 이미지와 생성 프롬프트](docs/campus-art-prompts.md)

3분 발표를 위해 인물당 대화 약 22초, 전체 약 2분 40초를 목표로 압축했다. 선택은 총 20번에서 8번으로 줄였다. 발표에서는 각 질문의 답 하나와 최종 엔딩 하나만 진행한다. 강제 타이머 없이 플레이어가 읽고 선택하며 사주 노트 열람 시간에 따라 달라진다. 실제 역할은 정치훈 PM, 박진환 백엔드, 주정원 프론트엔드, 남성수 발표로 유지하고, 게임 속 과장된 캐릭터와 마지막 역할 공개 문구를 연결한다.

현대 캠퍼스 이미지, 교복·니트 차림의 AI 인물 4장, 큰 인물과 PC 27px·모바일 23px 대사를 유지한다. 원본 인물 사진은 공개 배포에 포함하지 않는다.

[배경음악](docs/background-music.md)은 SOUNDSPRAY의 **Sweet Cherry**다. 첫 화면 조작 후 기본 20% 음량으로 반복 재생하며, 게임·팀 소개 우측 상단의 음악 버튼에서 소리 켜기/끄기와 음량을 조절한다. 설정은 다음 방문에도 유지한다.

[로판 서체 구성](docs/typography.md): 제목·인물 이름은 Hahmlet, 대사·본문은 마루부리, 메뉴는 고운돋움, 영문 장식은 Cormorant Garamond를 사용한다. 게임·설정·저장·사주 노트·팀 소개까지 적용하며 웹폰트와 라이선스를 사이트에 포함한다.

선택·되감기·대사록·저장·불러오기는 공식 엔진이 처리한다. 새 분기는 `ASPS_TEAM01_kokone_short_v4` 저장 공간을 사용하고 이전 5문답판과 20컷 대본의 저장을 별도로 보관한다. 대화 답변, 만남 완료 순서, 최종 선택, 사주 노트를 저장하고 새 회차에는 초기화한다.

성수의 두 번째 대화 뒤 `/api/team`으로 실제 사주 여섯 조합을 조회한다. `saju_score`만 사용하고 종합 점수와 혼합하지 않는다. 노트에는 서버 점수만 넣고 수치 설명은 한 화면으로 모았다. 주인공의 생년월일·사주는 설정하지 않는다. 조회 실패 시 점수를 만들지 않고 이야기를 계속하며 새 회차에서 다시 조회한다.
...
`GCS_DATASET_ID`로 API가 조회할 버전을 선택한다. 이름·순위·점수는 원점수 기준이다. API의 종합 최고점은 공동 1위가 있으면 이름순 한 조합과 공동 1위 수를 반환한다. 게임의 사주 노트는 `public/saju.js`에서 네 명의 12개 방향별 결과를 검증하고 여섯 조합으로 묶으며, 원점수로 정렬한 모든 조합을 노트에 표시한다. DB 수정 결과는 새 게임 회차에서 반영된다.
