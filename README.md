...
일상 선택은 반응과 호감도만 바꾼 뒤 같은 스토리로 합류한다. CUT 05·08·11·14에서 네 명의 프로필을 해금하고, CUT 19에서는 호감도와 관계없이 네 명 모두 선택할 수 있다. CUT 20의 4개 엔딩 뒤 캠퍼스 일러스트·팀 역할과 MEMBER PROFILE / OUR PROJECT / GITHUB를 제공하는 `/team.html`로 이어진다.

[전체 20컷 시나리오](docs/scenario-team04.md) · [엔진 및 편집 안내](docs/design-reference.md) · [캠퍼스 이미지와 생성 프롬프트](docs/campus-art-prompts.md)

고급 사립학교 청춘 드라마 분위기로 교복·니트 차림의 인물 4장, 학교 정문·프로젝트실·카페테리아·밤 교실, 네 사람이 함께 등장하는 표지를 내장 image_gen으로 제작했다. 네이비·화이트·버건디와 단정한 사각 프레임으로 게임과 엔딩 화면을 통일했다. 대사 글자는 PC 27px·모바일 23px이며 작은 대화창과 큰 인물 구도를 유지한다. 원본 인물 사진은 공개 배포에 포함하지 않는다.
